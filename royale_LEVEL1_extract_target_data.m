...
target_data_table = create_tablle_target_data(); 
...
function [t3] = create_tablle_target_data()
% set up output file 
uuii = zeros(1, 34);
...
%     viscircles(centers, radii,'EdgeColor','g');
    ...
    if ~isempty(centers)
        ...
        elseif t_distance == 1.45 && ~length(radii(:,1))==2
            disp("error reading 1.6 sphere " + string(iFrame))
        end
    end    
end
new_table = t3;
